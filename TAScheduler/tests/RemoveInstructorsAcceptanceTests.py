...
class testPositive(TestCase):
    ...
    def test_changeInstructor(self):
        resp = self.mockClient.post("/assignInstructor/", {"InputCourseCode": "1", "InputIDNumber": "1"}, follow=True)
    ...
    def test_removeInstructor(self):
        resp = self.mockClient.post("/unassignInstructor/", {"InputCourseCode": "1"}, follow=True)
        checkCourse = Course.objects.get(courseCode="1")
...
class testNegative(TestCase):
    ...
    def test_unassignNotFullyFilledOut(self):
        ...
        resp = self.mockClient.post("/unassignInstructor/", {"InputSectionCode": ""}, follow=True)
        checkCourse = Course.objects.get(courseCode="")
        self.assertNotIn("", checkCourse, "Item was not added to list")
        resp = self.mockClient.post("/unassignInstructor/", {"InputCourseCode": None}, follow=True)
    ...
    def test_NoInstructorToRemove(self):
        ...
        resp = self.mockClient.post("/unassignInstructor/", {"InputCourseCode": "3"}, follow=True)
    ...
    def test_noInstructorAttached(self):
        self.mockClientclient.post("/", {"InputCourseCode": "2", "InputIDNumber": "1"}, follow=True)
        resp = self.mockClient.post("/assignInstructor/", {"InputCourseCode": "1"})
        checkCourse = Course.objects.get(courseCode="1")
        self.assertNotIn("300", checkCourse, "Item was missing Instructor ID")

    def test_noCourseEntered(self):
        self.mockClientclient.post("/", {"InputCourseCode": "2", "InputIDNumber": "1"}, follow=True)
        resp = self.mockClient.post("/unassignInstructor/", follow=True)
        checkCourse = Course.objects.get(courseCode="")
